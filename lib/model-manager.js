...
class ModelManager extends EventEmitter {
  constructor(options = {}) {
    super();
    const homeDir = process.env.COPILOT_HOME || (process.platform === 'win32' ? process.env.USERPROFILE : process.env.HOME);
    const copilotDir = path.join(homeDir, '.copilot');
    
    this.modelPath = options.modelPath || path.join(copilotDir, 'models');
    this.configPath = options.configPath || path.join(copilotDir, 'models.json');
    this.trainingPath = options.trainingPath || path.join(copilotDir, 'training');
   ...
   */
  _validateConfig() {
    const required = ['models', 'settings', 'gdrive'];
    ...
    ifif (!modelName || typeof modelName !== 'string') {
